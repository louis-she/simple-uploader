...
func uploadSlice(slice int64, meta controllers.FileMeta, file *os.File, assert *assert.Assertions, v string) *httptest.ResponseRecorder {
	...
	var path string
	if v == "v2" {
		path = "/files/" + meta.FileId + "/upload_v2"
	} else {
		path = "/files/" + meta.FileId + "/upload"
	}
	req, _ := http.NewRequest("POST", path, multipartBody)
...
func TestCreateFileNoArgsV2(t *testing.T) {
	assert := assert.New(t)
	req, _ := http.NewRequest("POST", "/files_v2", nil)
	w := createFileWithRequest(req)
	assert.Equal(http.StatusBadRequest, w.Code)
}
...
func TestCreateFileInvalidArgsV2(t *testing.T) {
	assert := assert.New(t)
	...
	body, _ := json.Marshal(params)
	req, _ := http.NewRequest("POST", "/files_v2", bytes.NewBuffer(body))
	w := createFileWithRequest(req)
	assert.Equal(http.StatusBadRequest, w.Code)
}
...
func TestCreateWithDoubleDotPrefixV2(t *testing.T) {
	assert := assert.New(t)
...
func TestCreateFileValidV2(t *testing.T) {
	assert := assert.New(t)
	file := generateRandomLargeFile(0)
	defer os.Remove(file.Name())
...
func TestFileUploadSingle(t *testing.T) {
	...
	// upload
	w := uploadSlice(0, responseMeta, file, assert, "v1")
	assert.Equal(http.StatusOK, w.Code)
...
func TestFileUploadSingleV2(t *testing.T) {
	assert := assert.New(t)
	file, responseMeta := createRandomFile(0, 10*1024*1024)
	defer os.Remove(file.Name())

	// upload
	w := uploadSlice(0, responseMeta, file, assert, "v2")
	assert.Equal(http.StatusOK, w.Code)
	...
	slicePath := path.Join(viper.GetString("uploader.slice_cache_dir"), responseMeta.FileId, "meta.json")
	assert.FileExists(slicePath)
}
...
func TestFileUploadWithPrefixV2(t *testing.T) {
	...
	uploadSlice(0, responseMeta, file, assert, "v2")
	assert.FileExists(path.Join(viper.GetString("uploader.upload_dir"), "test_prefix", responseMeta.FileName))
}
...
func TestFildUploadMultipleSlices(t *testing.T) {
	...
	for i := 0; i < slicesNum; i++ {
		// upload
		w := uploadSlice(int64(i), responseMeta, fileReader, assert, "v1")
...
func TestFildUploadMultipleSlicesV2(t *testing.T) {
	assert := assert.New(t)
	file, responseMeta := createRandomFile(0, 0)
	defer os.Remove(file.Name())
	...
	for i := 0; i < slicesNum; i++ {
		// upload
		w := uploadSlice(int64(i), responseMeta, fileReader, assert, "v2")
		...
		if i != slicesNum-1 {
			...
			sha1Hex := hex.EncodeToString(sha1Sum[:])
			cacheSlicePath := path.Join(slicesDir, responseMeta.FileName+"."+strconv.Itoa(i)+"."+sha1Hex+".slice")
			assert.NoFileExists(cacheSlicePath)
		} else {
			destFilePath := path.Join(viper.GetString("uploader.upload_dir"), responseMeta.FileName)
			assert.Equal(http.StatusOK, w.Code)
			assert.DirExists(slicesDir)
			assert.FileExists(destFilePath)
			// compare sha1
			localBytes := make([]byte, responseMeta.FileSize)
			file.Seek(0, 0)
...
func TestFileUploadInteruptResume(t *testing.T) {
	...
	uploadSlice(0, responseMeta, file, assert, "v1")
	uploadSlice(2, responseMeta, file, assert, "v1")
	...
	for _, slice := range meta.Slices {
		...
		uploadSlice(int64(sliceId), responseMeta, file, assert, "v1")
	}
...
func TestFileUploadInteruptResumeV2(t *testing.T) {
	assert := assert.New(t)
	file, responseMeta := createRandomFile(0, 0)
	defer os.Remove(file.Name())

	uploadSlice(0, responseMeta, file, assert, "v2")
	uploadSlice(2, responseMeta, file, assert, "v2")
	...
	for _, slice := range meta.Slices {
		...
		sliceId, _ := strconv.Atoi(slice.Id)
		uploadSlice(int64(sliceId), responseMeta, file, assert, "v2")
	}

	// all file uploaded
	assert.DirExists(path.Join(viper.GetString("uploader.slice_cache_dir"), responseMeta.FileId))
	destFilePath := path.Join(viper.GetString("uploader.upload_dir"), responseMeta.FileName)
	assert.FileExists(destFilePath)
